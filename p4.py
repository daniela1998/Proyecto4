...
Original file is located at
    https://colab.research.google.com/drive/1CZZHelOVikl_ZKmFN6pexqkwwQoI3uGB
...
def modulador(bits, fc, mpp):
    ...
    moduladora_Q = np.zeros(t_simulacion.shape)
...
"""Función Demoduladora -> Decodificación por detección de energía"""
...
"""Main -> Aquí se llaman a todas las funciones"""
...
AB = [-1,1]                     
iterar = 4                          
H_t = np.empty((iterar, len(t)))   
J_t = np.empty((iterar, len(t)))
...
# Datos
plt.title('Realizaciones de $X(t)$')
plt.xlabel('$t$')
plt.ylabel('$x_i(t)$')
plt.legend([Rea, Teo], ['Promedio', 'Valor teorico'])
plt.show()   
...
# Matriz de valores de correlación
corr = np.empty((iterar, len(des)))

# Figura autocorrelación
plt.figure()

# Cálculo de correlación
for n in range(iterar):
    for i, tau in enumerate(des):
...
plt.title('Funciones - Autocorrelación ')
plt.xlabel(r'$\tau$')
